...
const PMTableBiscuit = ({ group }) => {
  ...
  const fetchData = async ({ startIndex = 0, stopIndex = 20 } = {}) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://10.37.12.17:3000/api/pm_biscuit/${group}/${selectedYear}?start=${startIndex}&end=${stopIndex}`);
  ...
  const deletebiscuit = async () => {
    setLoading(true);
    handleCloseDialog();
    try {
      await axios.delete(`http://10.37.12.17:3000/api/deleted/biscuit/${group}`);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'PMTableBiscuit');
    XLSX.writeFile(workbook, `PMTableBiscuit_Group${group}.xlsx`);
  };
    ...
    <MainCard title={`PMLine ${group} demo format periode start(2024w3), periode(L10w)`} sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogActions>
          ...
          <Button onClick={deletebiscuit} color="error" autoFocus>Hapus</Button>
        </DialogActions>
      </Dialog>
