...
const FormLHP = ({ isLoading }) => {
    ...
    useEffect(() => {
        const { wikulitawal, wikulitakhir, batch_buat, beratKering } = formState;
        // Jika tidak ada nilai, hentikan eksekusi
        if (!wikulitawal && !wikulitakhir) {
            return;
        }
        const wipSelisih = parseFloat(wikulitakhir || 0) - parseFloat(wikulitawal || 0);
    ...
    }, [formState.wikulitawal, formState.wikulitakhir]);
            ...
            {isLoading ? (
                ...
                <MainCard content={false}>
                    <CardContent>
                        ...
                        <Grid container spacing={2} sx={{ mt: 2 }}>
                            <Grid item xs={12} sm={4}>{renderTextField('WIP Kulit Awal', 'wikulitawal')}</Grid>
                            <Grid item xs={12} sm={4}>{renderTextField('WIP Kulit Akhir', 'wikulitakhir')}</Grid>
