...
const TableWeeklyPL6 = () => {
  const [value, setValue] = React.useState('packing_l6_weekly');
  ...
  const [urlapi] = useState({
    ...
    packing_l6_weekly : 'http://10.37.12.17:3000/packing_l6_weekly',
    urlp2 : 'table',
  ...
  const handleChange = (event, newValue) => {
    ...
    if(newValue=='packing_l6_weekly'){
        axios.get(urlapi.packing_l6_weekly)
          .then(response => {
  ...
  <MainCard title="Packing Wafer Line 7" secondary={<ButtonBack path={'/utils/packing-Line7/table'} />} >
    <TabContext value={value}>
        ...
        <Grid item xs={12} sm={10}>
          <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
            <TabList onChange={handleChange} aria-label="lab API tabs example">
                <Tab label="Home" value="packing_l6_weekly" />
                <Tab label="B1" value="packing_b1_weekly" />
                <Tab label="B2" value="packing_b2_weekly" />
                <Tab label="B3" value="packing_b3_weekly" />
                <Tab label="B4" value="packing_b4_weekly" />
                <Tab label="A4" value="packing_a4_weekly" />
            </TabList>
          </Box>
        </Grid>
                ...
                <div>
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      let objects = [];  
                      let day = ['Monday','Tuesday','Wednesday','Thursday' , 'Friday' , 'Saturday','Total'];
                      if (value==='packing_l6_weekly') {
                        axios.get(`http://10.37.12.17:3000/packing_l6_weekly/date/${weekly}`)
                        .then(response => {
                        var dataolah  = response.data;
                        var datanih1  =   dataolah[0] === undefined ? 0 : dataolah[0].cntr_bandet 
                        var datanih2  =   dataolah[1] === undefined ? 0 : dataolah[1].cntr_bandet
      ...
      <TabPanel value="packing_l6_weekly">
         <DataGrid rows={data} columns={fieldtwo} loading={isLoading} slots={{ toolbar: GridToolbar }} />
      </TabPanel>
      <TabPanel value="packing_b1_weekly">
        <Grid container spacing={gridSpacing}>
          <DataGrid rows={data} columns={field} loading={isLoading} slots={{ toolbar: GridToolbar }} />
        </Grid>
      </TabPanel>
      <TabPanel value="packing_b2_weekly">
        <Grid container spacing={gridSpacing}>
          <DataGrid rows={data} columns={field} loading={isLoading} slots={{ toolbar: GridToolbar }} />
        </Grid>
      </TabPanel>
      <TabPanel value="packing_b3_weekly">
        <Grid container spacing={gridSpacing}>
          <DataGrid rows={data} columns={field} loading={isLoading} slots={{ toolbar: GridToolbar }} />
        </Grid>
      </TabPanel>
      <TabPanel value="packing_b4_weekly">
        <Grid container spacing={gridSpacing}>
          <DataGrid rows={data} columns={field} loading={isLoading} slots={{ toolbar: GridToolbar }} />
        </Grid>
      </TabPanel>
      <TabPanel value="packing_a4_weekly">
        <Grid container spacing={gridSpacing}>
          <DataGrid rows={data} columns={field} loading={isLoading} slots={{ toolbar: GridToolbar }} />
        </Grid>
      </TabPanel>
